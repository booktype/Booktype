...
$(function() {
	...
	function showJoined(notice) {
	    ...
	    msg.find('.notice').html(escapeJS(notice));
	...
	function showInfo(notice) {
		...
		//msg.find('.notice').html(notice.message);
	    } else {
		msg.find('.notice').html(formatString($.booki._('message_info_'+notice.message_id, ''), notice.message_args));
	    }
	...
	function formatMessage(from, message) {
	    return $("<p><b>"+from+"</b>: "+unescapeHtml(message)+"</p>");
	}
	...
	function initUI() {
	    element.html($('<form onsubmit="javascript: return false;"><div class="content" style="margin-bottom: 5px; width: 265px; height: 300px; border: 1px solid black; padding: 5px"></div><input type="text" style="width: 275px;"/></form>').submit(function() { var s = $("INPUT", element).val(); $("INPUT", element).removeAttr("value"); showMessage($.booki.username, s);  $.booki.sendToChannel("/chat/"+$.booki.currentBookID+"/",{"command": "message_send", "message": s}, function() {} );
}));
	}
	...
	jQuery.booki.editor = function() {
		...
		'refresh': function() {
		    ...
		    $.each(this.items, function(i, v) {
			$("#item_"+v.id+"  .title").html(escapeJS(v.title));
			$("#item_"+v.id+" n .status").html('<a href="javascript:void(0)" onclick="$.booki.editor.editStatusForChapter('+v.id+')">'+escapeJS(getStatusDescription(v.status))+'</a>');
		    });
		...
		'setHistoryView': function() {
			...
			$.each(data.history, function(i, entry) {
				...
				$.booki.ui.fillTemplate(en, {"entryKind": kindName,
				  	                     "entryChapter": escapeJS(entry.chapter),
					                     "entryUser": escapeJS(entry.user),
				...
				var en = $.booki.ui.getTemplate('rowClone');
				$.booki.ui.fillTemplate(en, {"entryKind": kindName,
					    "entryChapter": escapeJS(entry.chapter),
				...
				s.append(en);
			    } else if(entry.kind == 'cover_upload') {
				var en = $.booki.ui.getTemplate('rowCoverUpload');
				var fname = entry.args.title || entry.args.filename;
				
				$.booki.ui.fillTemplate(en, {"entryFilename": '"'+shortenString(fname, 40)+'"',
					    "entryUser": escapeJS(entry.user),
					    "entryModified": entry.modified});

				s.append(en);
			    } else if(entry.kind == 'cover_delete') {
				var en = $.booki.ui.getTemplate('rowCoverDelete');
				var fname = entry.args.title || entry.args.filename;
				...
				s.append(en);
			    } else if(entry.kind == 'cover_update') {
				var en = $.booki.ui.getTemplate('rowCoverUpdate');
				var fname = entry.args.title || entry.args.filename;
				...
				s.append(en);
			    } else if(entry.kind == 'chapter_delete') {
				var en = $.booki.ui.getTemplate('rowChapterDelete');
				var fname = entry.args.chapter;
			...
			his.append("<br/>");
			his.append($('<a href="javascript:void(0)">&lt;&lt;'+$.booki._('previous', 'Previous')+'</a>').click(function() {
			    if(_page < 2) return;
			...
			his.append("&nbsp;&nbsp;");
			his.append($('<a href="javascript:void(0)">'+$.booki._('next', 'Next')+'&gt;&gt;</a>').click(function() {
				    $.booki.ui.notify($.booki._('reading_history_data', 'Reading history data...'));
				    $($this.containerName+" TABLE").css("background-color", "#f0f0f0");
...
//							  s.append($("<tr><td>"+entry.kind+'</td><td><a href="'+$.booki.getBookURL()+entry.chapter_url+'/" style="text-decoration: underline">'+entry.chapter+"</a></td><td>"+entry.user+"</td><td>"+entry.modified+"</td><td></td></tr>"));
		...
		'setRevisionView': function(chapterName, chapterURL, chapterHistory, revision) {
			...
			disp.find('.chapterName').html(escapeJS(chapterName));

			disp.find('.dataUser').html(escapeJS(data.user));
			disp.find('.dataModified').html(escapeJS(data.modified));
			disp.find('.dataVersion').html(escapeJS(data.version));
			disp.find('.dataRevision').html(escapeJS(data.revision));
			if(data.comment) {
			    disp.find('.revisiondCcomment').show();
			    disp.find('.dataComment').html(escapeJS(data.comment));
			} else {
		...
		'setCompareView': function(chapterName, chapterURL, chapterHistory, revision1, revision2) {
			...
			disp.find('.backLink').click(function() { $this.setChapterView(chapterName, chapterURL, chapterHistory);});
			disp.find('.revision1Name').html(escapeJS(revision1));
			disp.find('.revision2Name').html(escapeJS(revision2));
		...
		line.find('.title').html(escapeJS(name));
		return line;
	    }
	    ...
	    */
		...
		line.attr('id', 'item_'+chapterID);
		$.booki.ui.fillTemplate(line, {'title': escapeJS(name)});
		...
		line.find('.lockUser').click(function() {
		...
		if(!isAdmin) 
		    line.find('.deletechapter').hide();

		line.find('.deletechapter').click(function() {
		    var disp = $.booki.ui.getTemplate('chapterDeleteDialog');
		    
		    disp.dialog({modal: true,
				 width: 450, 
				 create: function() {
				     var $dialog = $(this);

				     $(".remove_chapter_button").button("disable");

				     $("INPUT[name=agree]", $dialog).change(function() {
					 var $elem = $(this);

					 if($elem.is(":checked")) {
					     $(".remove_chapter_button").button("enable");
					 } else {
					     $(".remove_chapter_button").button("disable");
					 }
				     });
				 ...
				 buttons: [
				     ...
				     {text: $.booki._('remove_chapter', 'Delete chapter'),
				      class: 'remove_chapter_button',
				      disabled: true,
				      click:   function() {
					  var $dialog = $(this);

					  $.booki.sendToCurrentBook({"command": "chapter_delete", "chapterID": chapterID}, function() { 
					      $dialog.dialog('close');
					  });
				      }
				     }
		...
		line.dblclick(function() {
		    ...
		    var $this = $(this);
                    ...
                    $(".cont", $this).html($('<form method="GET" action="javascript:void(0)" style="font-size:12px; white-space: nowrap"></form>'));

		    // Make it look nicer and look on the width of input box
		    // also use white-space: nowrap

		    var e = $('<input type="text" style="width: 70%">').attr("value", s);
		    // bug with field selection in firefox
		    e.mousedown(function(e){ e.stopPropagation(); });
			...
			return false;
                    ...
                    $("FORM", $this).append($('<a href="#">'+$.booki._('SAVE', 'SAVE')+'</a>').click(_doRename));
		    $("FORM", $this).submit(_doRename);
	    ...
	    */
	    ...
	    function _showCovers() {
		switch(coverFilter) {
		    case "approved":
		        $("#cover_manager .cover.coverApproved").css("display", "");
		        $("#cover_manager .cover.coverPending").css("display", "none");
		        break;
		    
		    case "pending":
		        $("#cover_manager .cover.coverApproved").css("display", "none");
		        $("#cover_manager .cover.coverPending").css("display", "");
		        break;

		    case "all":
		        $("#cover_manager .cover").css("display", "");
		        break;
		}
	    ...
	    function _loadCovers() {
		$.booki.ui.notify($.booki._('loading_data', 'LOADING DATA....'));
		
		$.booki.sendToCurrentBook({"command": "covers_data"},
					      function(data) {
						  $.booki.ui.notify();

						  $("#covers").empty();
						  // turn on/off approved/pending ...
						  // sa classom pa da bude hidden?!
						  // prikazuje ili ne prikazuje, ovisi
						  ...
						  $.each(data.covers, function(i, v) {
						      var c = $.booki.ui.getTemplate('coverThumbnail');
						      $.booki.ui.fillTemplate(c, {"title": escapeJS(v.title)});

						      $("IMG", c).attr("src", "/"+$.booki.currentBookURL+"/_cover/"+v.cid+"/?preview=1");
						      
						      $("dd.usage", c).addClass(v.placement).attr("title", $.booki._(v.placement+'_cover', ''));

						      // back spine front whole
						      $("FORM A", c).click(function() {
							  if(confirm($.booki._('delete_cover', "Do you really want to delete this cover?")) == true) {
							      $.booki.sendToCurrentBook({"command": "cover_delete",
											 "cid": v.cid},
											function(dta) {
											    _loadCovers();
											});
						      ...
						      $("FORM label", c).attr("for", "cover"+i);
						      $("FORM INPUT", c).attr("id", "cover"+i);
						      
						      $("FORM BUTTON[name=edit]", c).click(function() {
							  jQuery.booki.sendToCurrentBook({"command": "cover_load",
											  "cid": v.cid},
											 function(dta) {
											     var disp = $.booki.ui.getTemplate('coverPanel');
											     $("IMG", disp).attr("src", "/"+$.booki.currentBookURL+"/_cover/"+v.cid+"/?preview=1");
											     $("A.downloadlink", disp).attr("href", "/"+$.booki.currentBookURL+"/_cover/"+v.cid+"/"+dta.cover.filename);
											     if(dta.cover.filetype == '') {
												 $("dd.filetype", disp).html($.booki._('unknown', 'Unknown'));
												 $("dd.preview_width", disp).html('?');
												 $("dd.preview_height", disp).html('?');
											     } else {
												 $("dd.filetype", disp).html(dta.cover.filetype);
												 $("dd.preview_width", disp).html(dta.cover.img_size[0]);
												 $("dd.preview_height", disp).html(dta.cover.img_size[1]);
											     }

											     $("INPUT[id=title]", disp).val(escapeJS(dta.cover.title));
											     
											     $("INPUT[id=filename]", disp).val(escapeJS(dta.cover.filename));

											     if(dta.cover.booksize == 'custom')
												 $(".customsize", disp).show();
											     else
												 $(".customsize", disp).hide();

											     $("OPTION[value="+dta.cover.booksize+"]", disp).attr('selected', 'selected');

											     $("INPUT[id=width]", disp).val(escapeJS(dta.cover.width));
											     $("INPUT[id=height]", disp).val(escapeJS(dta.cover.height));


											     $("#booksize", disp).change(function() {
												 var $this = $(this);
											     ...
											     if(dta.cover.units != '')
												 $('#units .'+dta.cover.units, disp).attr('selected', 'selected');

											     $("INPUT[id=creator]", disp).val(escapeJS(dta.cover.creator));
											     $("TEXTAREA[id=notes]", disp).val(escapeJS(dta.cover.notes));
											     											     
											     $.each(dta.licenses, function(a, b) {
												 $("SELECT[id=license]", disp).append('<option value="'+b[0]+'">'+b[1]+'</option>');
											     });

											     $("OPTION[value="+dta.cover.license+"]", disp).attr("selected", "selected");
											     $("SELECT[id=type] OPTION[value="+dta.cover.cover_type+"]", disp).attr("selected", "selected");


											     if(dta.cover.format.indexOf("ebook") != -1)
												$("SELECT[id=format] OPTION[value=ebook]", disp).attr("selected", "selected");

											     if(dta.cover.format.indexOf("book") != -1)
												$("SELECT[id=format] OPTION[value=book]", disp).attr("selected", "selected");

											     if(dta.cover.format.indexOf("pdf") != -1)
												$("SELECT[id=format] OPTION[value=pdf]", disp).attr("selected", "selected");
											     ...
											     // 
											     $("BUTTON", disp).click(function() {
												 $("#cover_upload INPUT[type=file]").click();
												 return false;
											     });

											     disp.attr("id", "settings_pane");

											     disp.dialog({modal: false,
													  width: 450, 
													  buttons: [
													      {text: $.booki._('close', 'Close'),
													       click:   function() {
													      ...
													      {text: $.booki._('save_changes', 'Save changes'),
													       click:   function() {
														   var $dialog = $(this);
														   var f = $.makeArray($("#format :selected", disp).map(function() { return $(this).val(); })).join();
														   jQuery.booki.sendToCurrentBook({"command": "cover_save",
																		   "cid": v.cid,
																		   "title": $("INPUT[id=title]", disp).val(),
																		   "width": $("INPUT[id=width]", disp).val(),
																		   "height": $("INPUT[id=height]", disp).val(),
																		   "units": $("#units OPTION:selected", disp).val(),
																		   "booksize": $("#booksize option:selected", disp).val(),
																		   "format": f,
																		   "license": $("#license option:selected", disp).val(),
																		   "cover_type": $("#type option:selected", disp).val(),

																		   "notes": $("TEXTAREA", disp).val(),
																		   "creator": $("INPUT[id=creator]", disp).val()}, 
																		  function(data) {
																		      _loadCovers();
						      ...
						      $(["ebook", "book", "pdf"]).each(function(i, e) {							  
							  if(v.format.indexOf(e) == -1) {
							      $("DD.cover"+e, c).css("display", "none");
							  }							  
						      });
						      
						      if(!data.can_update)
							  $("FORM INPUT", c).attr("disabled", "disabled");

						      if(v.approved) {
							  $("FORM INPUT", c).attr("checked", "checked");							 
							  c.addClass("coverApproved");
						      } else {
							  c.addClass("coverPending");
						      }

						      $("INPUT[name=approved]", c).change(function() {
							  // must send message to the server
							  var isChecked = $(this).is(":checked");

							  if(isChecked) 
							      c.removeClass("coverPending").addClass("coverApproved");
							  else
							      c.removeClass("coverApproved").addClass("coverPending");

							  jQuery.booki.sendToCurrentBook({"command": "cover_approve",
											  "cid": v.cid,
											  "cover_status": isChecked},
											 function(dta) {
											     if(coverFilter != "all")
												 c.fadeOut('slow', function() { _showCovers(); });
											 });
	    ...
	    function _initUnsaved() {
		    ...
		    $(window).bind('beforeunload', function(e) {    
			    ...
			    return $.booki._('leaving_editor', 'You are leaving Booktype Editor.');
			});
	    }
	    ...
	    function coverPreviewSize(typ, width, height) {
		$("#cover_meta .filetype").html(typ);
		$("#cover_meta .preview_width").html(width);
		$("#cover_meta .preview_height").html(height);
	    }
		...
		editStatusForChapter: function(chapterID) {
		    ...
		    dial.find("UL").html($.map(statuses, function(i, n) { return '<li style="list-style: none; margin-left: 0;"><input name="statchap" id="statchap'+i[0]+'" type="radio" value="'+escapeJS(i[0])+'" /><label for="statchap'+i[0]+'">'+escapeJS(i[1])+'</label></li>'}).join(''));
		...
		viewChapter: function(chapterID) {
		    ...
		    $.booki.sendToCurrentBook({"command": "get_chapter", "chapterID": chapterID, "lock": false, "revisions": true}, 
						  ...
						  $dialog.dialog({
							      autoOpen: false,
							      modal: false,
							      title: $.booki._('booki_reader', 'Booktype Reader'),
							      height: 530,
							      width:  600
							  });
						  $dialog.dialog('open');
		...
		editChapter: function(chapterID) {
		    ...
		    $.booki.sendToCurrentBook({"command": "get_chapter", "chapterID": chapterID}, function(data) {
						  ...
						  $("#editorcontainer").html(ed).css("display", "block").fadeIn("slow", function() {
						         /* wait for TinyMCE to be initialised before setting the text */
						          window.EVIL_TINY_OPTIONS.oninit = function() {
							  ...
							  $("#editorcontainer .title").html(escapeJS(data.title)); 
		...
		showSettingsTab: function() {
		    ...
		    $.booki.sendToCurrentBook({"command": "settings_options"},
						  ...
						  var s = $.map(data['licenses'], function(i, n) { return '<option value="'+i[0]+'">'+escapeJS(i[1])+'</option>';}).join('');
						  $("#tabsettings SELECT[name='licen']").html(s);

						  $("#tabsettings SELECT[name='licen']").find('OPTION[value="'+escapeJS(data['current_licence'])+'"]').attr('selected', 'selected');

						  var s2 = $.map(data['languages'], function(i, n) { return '<option value="'+i[0]+'">'+escapeJS(i[1])+'</option>';}).join('');
						  $("#tabsettings SELECT[name='lang']").html(s2);

						  $("#tabsettings SELECT[name='lang']").find('OPTION[value="'+escapeJS(data['current_language'])+'"]').attr('selected', 'selected');

						  if(data['rtl'] == "RTL")
						      $("#tabsettings INPUT[name=rtl]").attr('checked', 'checked');


						  var s = $.map(statuses, function(i, n) { return '<i>'+escapeJS(i[1])+'</i>';}).join(', ');
		...
		showAttachmentsTab: function() {
		    ...
		    $.booki.sendToCurrentBook({"command": "attachments_list"},
						  ...
						  $.each(data.attachments, function(i, entry) {
							  s.append('<tr><td width="30"><input type="checkbox" name="'+entry.id+'" value="'+escapeJS(entry.name)+'"></td><td><a style="text-decoration: underline" href="'+$.booki.utils.linkToAttachment($.booki.currentBookURL, entry.name)+'" target="_new">'+$.booki.utils.maxStringLength(entry.name, 30)+'</a></td><td>'+$.booki.utils.formatDimension(entry.dimension)+'</td><td style="white-space: nowrap">'+$.booki.utils.formatSize(entry.size)+'</td><td style="white-space: nowrap">'+entry.created+'</td></tr>');
						  });
		...
		_initSettingsUI: function() {
			   ...
			   dial.find("SELECT[name=listroles]").change(function() {
				   ...
				   if(newRole.val() != 0) {
				       ...
				       $.booki.sendToCurrentBook({"command": "roles_list",
						   "role": selectedRole},
					   function(data) {
					       var s = $.map(data['users'], function(i, n) { return '<option value="'+escapeJS(i[0])+'">'+escapeJS(i[1])+'</option>';}).join('');
		   ...
		   $("#tabsettings A[name=attribution]").button().click(function() {
			   ...
			   $.booki.sendToCurrentBook({'command': 'license_attributions'}, 
							 ...
							 var s = $.map(data['users'], 
								       function(i, n) { 
									   return '<option value="'+escapeJS(i[0])+'">'+escapeJS(i[0])+'  ('+escapeJS(i[1])+')</option>';
								       }).join('');
		   ...
		   $("#tabsettings A[name=chapterstatus]").button().click(function() {
				...
				return '<li class="ui-state-default" id="liststatus_'+i0+'"><span class="ajkon ui-icon ui-icon-arrowthick-2-n-s"></span><span class="statuslabel">'+escapeJS(i1)+'</span><div style="float: right"><a  class="lnkdel" href="#" name="'+i0+'"><span class="ui-icon ui-icon-closethick"></span></a></div><div style="float: right"><a  class="lnkedit" href="#" name="'+i0+'"><span class="ui-icon ui-icon-pencil"></span></a></div></li>';
			    }
		   ...
		   // "Read full text of the license" link
		    $("#tabsettings A[name=readlicense]").click(function() {
			    var LICENSE = {"CC BY":  "http://creativecommons.org/licenses/by/2.0/",
			    ...
			    var _v = $("#tabsettings SELECT[name=licen]").val();
				...
				$d.find('iframe').attr("src", LICENSE[_v]); 
			    } else {
				var _n = $("#tabsettings SELECT[name=licen]").val();

				var $d = $('<div title="'+_n+'"><p>'+$.booki._('no_license_text', 'I can not find description for this license.')+'</p></div>');
				$d.dialog({
					modal: true,
					    buttons: {
					    Ok: function() {
		...
		_initCoversUI: function() {
		    /* Covers */

		    var _DO_COVERS = false;
		    
		    $(document).ajaxComplete(function() {
			/* this is not the nicest thing */
			if(_DO_COVERS) {
			    _loadCovers();
			}
		    ...
		    $("#cover_meta BUTTON[name=cancel]").click(function() {
			$("#cover_meta").hide();
		    });

		    $("#cover_meta BUTTON[name=save]").click(function() {
			var $form = $("#cover_upload");
			var $cur  = $("#cover_meta FORM");

			var f = $.makeArray($("#format :selected", $cur).map(function() { return $(this).val(); })).join();

			$("INPUT[name=filename]", $form).val($("#filename", $cur).val());
			$("INPUT[name=creator]", $form).val($("#creator", $cur).val());
			$("INPUT[name=title]", $form).val($("#title", $cur).val());
			$("INPUT[name=booksize]", $form).val($("#booksize option:selected", $cur).val());
			$("INPUT[name=width]", $form).val($("#width", $cur).val());
			$("INPUT[name=height]", $form).val($("#height", $cur).val());
			$("INPUT[name=units]", $form).val($("#units option:selected", $cur).val());
			$("INPUT[name=format]", $form).val(f);
			$("INPUT[name=type]", $form).val($("#type option:selected", $cur).val());
			$("INPUT[name=license]", $form).val($("#license", $cur).val());
			$("INPUT[name=notes]", $form).val($("#notes", $cur).val());
			_DO_COVERS = true;

			$form.submit();
			$("#cover_meta BUTTON[name=save]").attr('disabled','disabled');
			return false;
		    });

		    $("#cover_upload INPUT[name=cover]").change(function(evt) {
			var $this = this;
			$.booki.sendToCurrentBook({"command": "cover_upload"},
						  function(data) {
						      $("#cover_meta BUTTON[name=save]").removeAttr('disabled');
						      $("#cover_meta div.message").hide();
						      $("#cover_meta div.messageratio").hide();
						      ...
						      $("#format :selected", $cur).removeAttr("selected");
						      var $meta =  $("#cover_meta");
						      
						      $("SELECT[id=license]", $meta).empty();
						      $.each(data.licenses, function(a, b) {
							  $("SELECT[id=license]", $meta).append('<option id="'+b[0]+'">'+b[1]+'</option>');
						      });


						      $(".customsize", $meta).hide();

						      $("#booksize", $meta).change(function() {
							  var $this = $(this);

							  if($this.val() == "custom") {
							      $(".customsize",  $meta).show();
							  } else {
							      $(".customsize",  $meta).hide();
							  }
						      });						      
							  ...
							  var allowedExtensions = ['.png', '.jpg', '.jpeg', '.jpe', '.gif', '.tiff', '.tif', '.xcf', '.psd', '.pdf', '.ps', '.svg', '.eps', '.ai', '.indd'];
							  ...
							  for(var i=0; i<allowedExtensions.length; i++) {
							      if(endsWith(file.name.toLowerCase(),  allowedExtensions[i])) isAllowed = true;
							  }

							  if(!isAllowed) {
							      $("#cover_error div").hide();
							      $("#cover_error div.wrong-extension").show();
							      $("#cover_error").show();
							      $meta.hide();
							      return;
							  }
							  ...
							  if(file.type == 'image/jpeg' || file.type == 'image/png' || file.type == 'image/gif') {
								  ...
								  $("#cover_meta IMG.imagepreview").attr("src", ev.target.result).load(function() {
								      var img = this; 
								      var typ;

								      switch(file.type) {
								      case 'image/jpeg': typ = 'JPEG'; break;
								      case 'image/png': typ = 'PNG'; break;
								      case 'image/gif': typ = 'GIF'; break;
								      default: typ = '';
								      } 

								      coverPreviewSize(typ, img.naturalWidth, img.naturalHeight);

								      if(img.naturalHeight/img.naturalWidth < 1) {
									  $("#cover_meta div.messageratio").show();
								      }

								      if((img.naturalWidth < 500 || img.naturalHeight < 500)) {
									  $("#cover_meta div.message").show();									 
								      }
								  });
							      }
							      $("#cover_meta #filename").val(file.name);
							      reader.readAsDataURL(file);
							  } else {
							      $("#cover_meta IMG.imagepreview").unbind('load');
							      var e = file.name.toLowerCase().split('.').pop();
							      var imageURL = '';

							      if(allowedExtensions.indexOf("."+e) == -1 || (e == 'tif' || e == 'tiff'))
								  imageURL = $.booki.bookViewUrlTemplate.replace('XXX', '../site_static/images/booktype-cover-error.png?');
							      else
   								  imageURL = $.booki.bookViewUrlTemplate.replace('XXX', '../site_static/images/booktype-cover-'+e+'.png?');

							      $("#cover_meta #filename").val(file.name);
							      
							      $("#cover_meta IMG.imagepreview").attr("src", imageURL)
							      coverPreviewSize($.booki._('unknown', 'Unknown'), '?', '?');
							  }
						      }
		    ...
		    // Initiate switch on top of the view

		    $("#cover_usage A").click(function() {
			var anchor = $(this);
			$("#cover_usage A").removeClass("active");
			anchor.addClass("active");

			coverFilter = anchor.attr("name");
		...
		_initUI: function() {
			...
			if(panelID == "tabattachments") {
			    $.booki.editor.showAttachmentsTab();
			}

			if(panelID == "tabcovers") {
			    $.booki.editor.showCoversTab();
			}

			if(panelID == "tabsettings") {
			    $.booki.editor.showSettingsTab();
			}

			if(panelID == "tabversions") {
			    ...
			    $.booki.sendToCurrentBook({"command": "get_versions"},
							  ...
							  $.each(data.versions, function(i, entry) {
							      ...
							      s += '<li><a target="_new" href="'+$.booki.getBookDraftURL(entry.major+'.'+entry.minor)+'">'+escapeJS(getVersionName(entry))+'   </a></li>';
							  });
			...
			var message = $.booki._('saving_notes', "Saving the book notes");
			...
			$.booki.sendToCurrentBook({"command": "notes_save", 'notes': new_notes },
                                                  ...
                                                  function(data) {
						      var message = $.booki._('notes_saved', "Book notes saved.");
			...
			$.map(options, function(i, v) {						
				if($('TEXTAREA[name='+i.name+']', frm).length > 0) {
				    $('TEXTAREA[name='+i.name+']', frm).html(escapeJS(i.value));
			...
			*/
		    ...
		    function createWizzard(wizzardName, wizzardType, wizzardData, theme) {
			var t = $(wizzardData.html);

			if(typeof theme != 'undefined') {
			    if(theme != 'style3') {
				$("#fonts.step", t).remove();
				$("#toggle3.toggle", t).remove();
				$("fieldset.advastyle", t).remove();
			    }
			}
			...
			$("FORM", t).formwizard({ 
			            textSubmit: $.booki._('submit', 'Submit'),
                                    textNext: $.booki._('next', 'Next'),
                                    textBack: $.booki._('back', 'Back'),
				    formPluginEnabled: true,
				    validationEnabled: false,
				    focusFirstInput : true,
                                    disableUIStyles: true, 
			            next: 'INPUT.buttonnext',
                                    back: 'INPUT.buttonback',
				    formOptions :{
					...
					beforeSubmit: function(dta) { 
						...
						if(dta[i].name == 'cover_image') {
						    coverData.push(dta[i].value);
						} else {
						    data.push(dta[i]);
						}
					    }
						...
						if(coverData.length == 1) 
						    data.push({'name': 'cover_image', 'value': coverData[0]});
						else
						    data.push({'name': 'cover_image', 'value': coverData});
					    }
			...
			setWizzardOptions($("FORM", t), wizzardData.options);
			    ...
			    $.booki.ui.notify($.booki._('loading_data_please_wait', 'Loading data. Please wait...'));
			    ...
			    $.booki.sendToCurrentBook({"command": "get_wizzard",
                    ...
                    $('A[name="bookjswizzard"]').button().click(function() {
			    $.booki.ui.notify($.booki._('loading_data_please_wait', 'Loading data. Please wait...'));

			    $.booki.sendToCurrentBook({"command": "get_wizzard",
 					               "wizzard_type": "bookjs"},
			    			      function(wizzardData) {
							  var theme = $('INPUT[name="style"]:checked').val();

							  createWizzard('bookjswizzard', 'bookjs', wizzardData, theme);							      
						      });
			});
		    ...
		    $('a[name=exportbutton]').button().click(function() {
			    ...
			    commands = {"command": "publish_book2",
					"publish_mode": publishMode}
                            ...
                            if(publishMode == 'bookjs') {
                                commands['theme'] = $('INPUT[name="style"]:checked').val();
                            }
			    ...
			    $.booki.sendToCurrentBook(commands,						  
							  ...
							  if(typeof data.covers == 'undefined')  {
							      ...
							      $.each(data.covers, function(i, c) {
								  if(i > 0)
								      $(".lst", t).append(", ");
								  $(".lst", t).append($('<a target="_new" href="'+c[1]+c[4]+'">'+(c[3] || c[4])+'</a>'));
							      });
							  }
		    ...
		    function _publishOptionSelected(opt) {
			if(opt == "bookjs") {
			    $("INPUT[name='style']").removeAttr("disabled");
			    $("DIV.styleoptions").removeClass("optionunselected");

			} else {
			    $("DIV.styleoptions").addClass("optionunselected");
			    $("INPUT[name='style']").attr("disabled", "disabled");
			}
		    }

		    $("INPUT[name='publish']").change(function() {
			_publishOptionSelected($("INPUT[name='publish']:checked").val());
		    });
		...
		drawAttachments: function() {
			...
			if(imageName.match(/.+\.jpg$/gi)) {
			    ...
			    $("#attachmentpreview").html('<img src="../_utils/thumbnail/'+escapeJS(imageName)+'"><br/><br/><a style="font-size: 10px" href="static/'+escapeJS(imageName)+'" target="_new">Open in new window</a>');
			}
		    });
		...
		*/		
		initEditor: function() {
			...
			// this does not work when you change chapter status very fast
				...
				holdChapters.delItemById(message.chapterID);
	...
	jQuery.booki.editor.upload = function() {
		...
		showFiles: function(func) {
			...
			if($.booki.utils.isImage(selectedFile))
			    $("#insertattachment .previewattachment").html('<img src="../_utils/thumbnail/'+escapeJS(fileName)+'">');
			else
		...
		redrawFiles: function() {
			...
			buttons: [
				  { text: $.booki._('insert_image', 'Insert image'),
				    click: function() {
				             if(selectedFile) {
				                editor.insertHTML('<img src="static/'+escapeJS(selectedFile)+'"/>');
				                $(this).dialog('close');
				             }
			             }
                                  },
