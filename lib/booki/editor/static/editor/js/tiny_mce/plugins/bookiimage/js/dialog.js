...
var ImageDialog = {
	init : function() {
	    ...
	    this.updateStyle();
	    this.showPreviewImage(true); // ovaj pravi probleme
	...
	showPreviewImage: function(doIUpdate) {
	    var src = document.forms[0].imageurl.value;
	    var to_update = doIUpdate || false;
	    if(src == '') return;
	    ...
	    if(to_update) s0 = ', true';
	    
	    var s = '<img src="'+imageURL+'" alt="Neki opis" onerror="ImageDialog.resetPreview();" onload="ImageDialog.updateImageData(this'+s0+');"/>';

	    j("#prev", j(document)).html(s);	
