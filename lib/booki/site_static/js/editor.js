...
/*						      s.append('<tr><td width="30"><input type="checkbox" name="'+entry.id+'" value="'+entry.name+'"></td><td><a style="text-decoration: underline" href="'+$.booki.utils.linkToAttachment($.booki.currentBookURL, entry.name)+'" target="_new">'+entry.name+'</a></td><td>'+$.booki.utils.formatDimension(entry.dimension)+'</td><td>'+$.booki.utils.formatSize(entry.size)+'</td><td>'+entry.created+'</td></tr>');*/
		...
		_initUI: function() {
		    ...
		    /* Button for publishing */
                    $('A[name="exportwizzard"]').button().click(function() {
                         var t = $.booki.ui.getTemplate("bookwizzard");
                         ...
                         $("FORM", t).formwizard({ 
                                       ...
                                       formOptions :{
                                             ...
                                             beforeSubmit: function(data) { alert("to je to"+$.param(data));/* $("#data").html("data sent to the server: " + $.param(data)); */},
                                             dataType: 'json',
                                             resetForm: true
                                        }	
                                       });
                         ...
                         $("FORM", t).bind("step_shown", function(event,data){	
		    ...
		    $('a[name=exportbutton]').button().button('disable');
		    $('FORM[name=formexport]', $('#tabpublish')).change(function() {
			    if($('INPUT:checked', $('FORM[name=formexport]')).length > 0) {
				$('a[name=exportbutton]').button("enable");
			    } else {
				$('a[name=exportbutton]').button("disable");
			    }
			});		    
