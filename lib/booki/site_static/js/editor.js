...
//							  s.append($("<tr><td>"+entry.kind+'</td><td><a href="'+$.booki.getBookURL()+entry.chapter_url+'/" style="text-decoration: underline">'+entry.chapter+"</a></td><td>"+entry.user+"</td><td>"+entry.modified+"</td><td></td></tr>"));
	    ...
	    */
		...
		editStatusForChapter: function(chapterID) {
		    ...
		    //var dial = $('.statusChapterDialog.template').clone().removeClass("template");
		...
		editChapter: function(chapterID) {
		    ...
		    $.booki.sendToCurrentBook({"command": "get_chapter", "chapterID": chapterID}, function(data) {
						  ...
						  $("#editor").css("display", "block").fadeIn("slow", function() {

						      });

						  // TODO, MUST CHECK THIS NOTIFICATION ERRORS
						  ...
						  */
						  ...
						  if(!_editorInitialized) {
						      $("#editor TEXTAREA").tinymce(window.EVIL_TINY_OPTIONS);
						      _editorInitialized = true;
						  } else {
						      $("#editor TEXTAREA").tinymce().show()
						  }
						  ...
						  $("#editor A[name=save]").unbind('click').click(function() {
							  ...
							  */
						      var content = $("#editor TEXTAREA").html();
						      /*
                                                      comment out spalato
...
img {\n\
  ...
  height: auto;\n\
		...
		showAdvancedPublishOptions: function(flag) {
		    if(flag) {
			$("#advancedswitch").html('<a href="javascript:void(0)" onclick="$.booki.editor.showAdvancedPublishOptions(false)">'+$.booki._('hide_advanced_options', 'Hide advanced options')+'</a>');

			$("#advanced", $("#tabpublish")).css("display", "block");
		    } else {
			$("#advanced", $("#tabpublish")).css("display", "none");

			$("#advancedswitch").html('<a href="javascript:void(0)" onclick="$.booki.editor.showAdvancedPublishOptions(true)">'+$.booki._('show_advanced_options', 'Show advanced options')+'</a>');
		    }
		},
		...
		showLuluFields: function() {
		    ...
		    $.booki.sendToCurrentBook({"command": "attachments_list"},
						  ...
						  var covers = $("#coverurl");
						  var s = '<option value="" selected="selected">None</option>';

						  $.each(data.attachments, function(i, entry) {
						      if (entry.name.substr(entry.name.length - 4) == ".pdf")
							  s = s + '<option value="'+$.booki.utils.linkToAttachment($.booki.currentBookURL, entry.name)+'">'+entry.name+'</option>';
/*						      s.append('<tr><td width="30"><input type="checkbox" name="'+entry.id+'" value="'+entry.name+'"></td><td><a style="text-decoration: underline" href="'+$.booki.utils.linkToAttachment($.booki.currentBookURL, entry.name)+'" target="_new">'+entry.name+'</a></td><td>'+$.booki.utils.formatDimension(entry.dimension)+'</td><td>'+$.booki.utils.formatSize(entry.size)+'</td><td>'+entry.created+'</td></tr>');*/
						  });
						  covers.html(s);
					      });
		...
		_initSettingsUI: function() {
		   ...
		   $("#tabsettings A[name=chapterstatus]").button().click(function() {
			    ...
			    //var dial = $('.statusChapterSettingsDialog.template').clone().removeClass("template");
		   ...
		   // "Read full text of the license" link
		    $("#tabsettings A[name=readlicense]").click(function() {
			    ...
			    // This is not the nices way to do it :)
		...
		_initUI: function() {
			...
			var isLulu = $("#tabpublish FORM INPUT[name='lulucom']").is(":checked");
			var publishMode = $("#tabpublish OPTION:selected").val();

			var coverUrl = "http://booki.flossmanuals.net"+$("#tabpublish #coverurl").val();
			var luluUser = $("#tabpublish FORM INPUT[name='luluuser']").val();
			var luluPassword = $("#tabpublish FORM INPUT[name='lulupassword']").val();
			var luluProject = $("#tabpublish FORM INPUT[name='luluproject']").val();
			...
			if(isLulu)
			    message += " as well as being published on Lulu.com";
			message += ".";
			...
			_incrementProgress();
                        ...
                        $.booki.sendToCurrentBook({"command": "publish_book",
						   ...
						   'cover_url': coverUrl,
						   'lulu_user': luluUser,
						   'lulu_password': luluPassword,
						   'lulu_project': luluProject,
						  },
