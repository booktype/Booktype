...
(function(win, jquery) {
    ...
    jquery.namespace('win.booktype.profile');
    
    win.booktype.profile = function() {
	...
	function _initUI() {
	    ...
	    $("FORM[name=formpassword]").submit(function() {
		    ...
		    if(password0 == '') {
			var txt = win.booktype._('passwordmissingold', '');
			...
			var txt = win.booktype._('passwordnotequal', '');
			$('DIV.notificationpassword').html('<p class="alert-box">'+txt+'</p>');
			...
			var txt = win.booktype._('passwordshort', '');
			$('DIV.notificationpassword').html('<p class="alert-box">'+txt+'</p>');
			$("INPUT[name=password1]", $frm).css('border', '1px solid red');
			$("INPUT[name=password2]", $frm).css('border', '1px solid red');
			return false;
		    ...
		    win.booktype.sendToChannel("/booki/profile/"+win.booktype.profileName+"/", 
					       {"command": "change_password",
						       "profile": win.booktype.profileName,
						       "password0": password0,
						       "password1": password1,
						       "password2": password2
						       },
						   ...
						   if(message.result == true) {
						       if(message.status == 1) {
							   var txt = win.booktype._('passwordchanged', '');
							   $('DIV.notificationpassword').html('<p class="alert-box">'+txt+'</p>');
						       ...
						       if(message.status == 2) {
							   var txt = win.booktype._('passwordwrongold', '');
							   $('DIV.notificationpassword').html('<p class="alert-box">'+txt+'</p>');
							   $("INPUT[name=password0]", $frm).css('border', '1px solid red');
						       }
						       if(message.status == 3) {
	...
	return {
		...
		win.booktype.subscribeToChannel("/booki/profile/"+win.booktype.profileName+"/", function(message) {
		    var funcs = {
			'user_status_changed': function() {
			    if(message.from == win.booktype.profileName) {
