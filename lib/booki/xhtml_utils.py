...
class ImportedChapter(BaseChapter):
    ...
    def localise_links(self):
        ...
        def localise(oldlink):
            ...
            target = '/media/%s.%s' % (target, ext)
            newlink = fetch_if_necessary(oldlink, target)
        ...
        tree.rewrite_links(localise, base_href=('http://%s/bin/view/%s/%s' % (self.server, self.book, self.name)))
        text = lxml.etree.tostring(tree.find('body'), method='xml')
        text = re.sub(r'^\s*<body.*?>\s*', '', text)
        text = re.sub(r'\s*</body>\s*$', '', text)
        self.text = text
        return self.text, images
