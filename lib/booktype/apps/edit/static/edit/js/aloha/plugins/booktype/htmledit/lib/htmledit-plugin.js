define(
  ['aloha', 'aloha/plugin', 'jquery19', 'ui/ui', 'aloha/ephemera', 'booktype', 'PubSub'],
  function (Aloha, Plugin, jQuery19, UI, Ephemera, booktype, PubSub) {
    'use strict';
    ...
    return Plugin.create('htmledit', {
      init: function () {
        ...
        jQuery19('#htmleditDialog').on('shown.bs.modal', function () {
          ...
          // autoformat to make the code readable
          var totalLines = myCodeMirror.lineCount();
          myCodeMirror.autoFormatRange({line: 0, ch: 0}, {line: totalLines});
        ...
        jQuery19('#htmleditDialog BUTTON.operation-set').on('click', function () {
          ...
          PubSub.pub('toolbar.action.triggered', {
            'event': event
          });
