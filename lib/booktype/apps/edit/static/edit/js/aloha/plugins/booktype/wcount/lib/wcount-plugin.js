define(
['aloha', 'aloha/plugin','aloha/ephemera', 'jquery','jquery19', 'ui/ui', 'ui/button', 'booktype' ],
...
//================
                                ...
                                function getDataFromServer(){
                                        var get_wordcount = function(callback) {
                                                booktype.sendToCurrentBook({"command": "word_count"}, 
                                                        function(data) {
                                                            console.log(data);
                                                            var all_wcount = 0;
                                                            var all_wordlist = 0;
                                                            ...
                                                            var all_content_text;
                                                            var all_wordlist;                                                            
                                                            var parser = new DOMParser;
                                                            var content = parser.parseFromString(data.data,"text/html");
                                                            all_words = get_text(content);
                                                            all_spacecount = jQuery(content).text().split(/\s+/).length-1;
                                                            all_content_text = jQuery(content).text();
                                                            all_wordlist = all_words.split(/\s+/);                      
                                                            ...
                                                            //outing the data
                                                            jQuery19("#elCount .modal-dialog .modal-content .modal-body .all_wcount").text(all_wcount);
                                                            jQuery19("#elCount .modal-dialog .modal-content .modal-body .all_charcount").text(all_charcount);
                                                            jQuery19("#elCount .modal-dialog .modal-content .modal-body .all_charspacecount").text(all_charspacecount);
