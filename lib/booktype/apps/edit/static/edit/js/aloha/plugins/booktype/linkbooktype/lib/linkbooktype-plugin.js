define(['aloha', 'aloha/plugin', 'jquery',  'jquery19', 'ui/ui', 'aloha/console', 'aloha/ephemera', 'booktype', 'underscore', 'PubSub', 'ui/scopes'], 
  function(Aloha, Plugin, jQuery,  jQuery19, UI, Console, Ephemera, booktype, _, PubSub, Scopes) {
    var $a = null,
      range = null,
      editable = null,
...
<a href='#' rel='popover' style= 'cursor: pointer' onclick=\"window.open('" + url + "','_blank')\">" + url + "</a></div>";
        ...
        jQuery19('#newLink BUTTON.operation-insert').on('click', function (event) {
          ...
          if(jQuery19.trim(title) !== '' && jQuery19.trim(url) !== '') {
            ...
            // publish message notification that link was creted
            PubSub.pub('toolbar.action.triggered', {'event': event});
          }
        });
      ...
      makeClean: function (obj) {    
        jQuery('div.popover.fade.top').remove();
      }
    });
  }
);
