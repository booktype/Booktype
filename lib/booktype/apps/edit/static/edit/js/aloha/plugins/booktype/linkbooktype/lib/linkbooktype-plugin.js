  define(['aloha', 'aloha/plugin', 'jquery',  'jquery19', 'ui/ui', 'aloha/console', 'aloha/ephemera', 'booktype', 'underscore'], 
  		function(Aloha, Plugin, jQuery,  jQuery19, UI, Console, Ephemera, booktype, _, bootstrap) {
			...
			function removeLinkToShowId(){
	  						console.log(Aloha.getActiveEditable());
							...
							rangeTree = range.getRangeTree(root);
							var i, rangeLength, content;
							jQuery.each(rangeTree,function(cnt,el){
								if(el.type==="none"){
									jQuery.each(el.children, function(cnt, element){
										if(jQuery(element.domobj).attr("id")==="linktoshow"){
											jQuery19("#linktoshow").tooltip("destroy");
											jQuery(element.domobj).removeAttr("id");
										}
									});
								}
							}); 
		        ...
		        init: function () {
		        	var isLinkSelected = false;
			  		Aloha.bind("aloha-link-selected",function(event){
			  			removeLinkToShowId(); // to remove any previous links that are shown
			  			var url = event.target.Selection.rangeObject.commonAncestorContainer.href;
						...
						if (root) {
							...
							var i, rangeLength, content;
							jQuery.each(rangeTree,function(cnt,el){
								var new_in_rangeTree = el.children;
								if(jQuery(el.domobj).attr("id")){
									jQuery19("#linktoshow").tooltip('show'); // show									
								} else {
									jQuery(el.domobj).attr("id","linktoshow");
									jQuery19("#linktoshow").tooltip({title:url}); // init
									jQuery19("#linktoshow").tooltip('hide'); // hide
								}
							});
						isLinkSelected = true;  
	  					};
