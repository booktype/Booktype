		        	...
		        	var selectedLinkDomObj = null;
			  		Aloha.bind("aloha-link-selected",function(event){
						...
						root = highestObject ? highestObject.parentNode : common;
							...
							var url = range.startContainer.parentNode.href;
							if(jQuery19("a.popover_shown").length===0){
								selectedLinkDomObj = range.startContainer.parentElement;								
								jQuery19(selectedLinkDomObj).popover({html: true, placement: 'top', content: "<a href='#' class='popover_shown' style='cursor:pointer;' onclick=\"window.open('"+url+"','_blank')\">"+url+"</a>"});
