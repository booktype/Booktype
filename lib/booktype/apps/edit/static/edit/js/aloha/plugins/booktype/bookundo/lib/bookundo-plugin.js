...
var $elements = range.getSelectionTree();
var selected;
$.each($elements,function(cnt,el){
	if(el.selection!=="none"){
		selected = el;
	}
});
...
var highestObject = GENTICS.Utils.Dom.findHighestElement(common, nodeName, limit);
var root = highestObject ? highestObject.parentNode : common;
var rangeTree = rangeObject.getRangeTree(root);

console.log(highestObject, root, rangeTree);
console.log(rangeObject, common, limit, markup, nodeName);

var sub_elements = getSiblings($(selected),[]);
console.log(sub_elements);
console.log(new_content.siblings);
console.log($node[0]);
console.log(getChildElement(sub_elements)[0]);
console.log(new_content.character);

newRange = new GENTICS.Utils.RangeObject();
// *** selecting the right part of the doc
//newRange.startContainer = newRange.endContainer = getChildElement(sub_elements)[0];
//newRange.startContainer = newRange.endContainer = $("#contenteditor").children()[1].children[9].children[3];
console.log($node[0]); */
console.log(new_content.siblings);
console.log($("#contenteditor").children()[1].children[9].children[3]);
newRange.startContainer = newRange.endContainer = $node[0];
// *** selecting the right part of the doc
...
/*                                      newRange.startOffset = selected_branch+1;
                                      ...
                                      newRange.startOffset = selected_branch;
                                      newRange.endOffset = selected_branch;
                                      newRange.select();
debugger;                                      
                                      Aloha.Selection.updateSelection();
                                      range = Aloha.Selection.rangeObject;
