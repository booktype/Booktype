...
define(['require','aloha','aloha/plugin','ui/ui','ui/button','aloha/rangy-core','jquery','undo/vendor/undo'],function (require) {
        ...
        plugin = Plugin.create('bookundo', {
               ...
               onEditableActivated: function (e,editable) {
                ...
                preSnapshot: function () {
                        var content;
                        if (myStack.position===0){ // store state before everything is done !!!
                            content = {
                                data: Aloha.getEditableById('contenteditor').originalContent,
                                position: {y: jQuery(window).scrollTop(),x: jQuery(window).scrollLeft()}
                            };
                            myStack.stack.push(content);
                            myStack.position++;
                        }
                        content = {
                            data: Aloha.getEditableById('contenteditor').getContents(),
                            position: {y: jQuery(window).scrollTop(),x: jQuery(window).scrollLeft()}
                        };
                        myStack.stack.push(content);
                        myStack.position++;
                 ...
                 */
                 // currently didn't do much work on that so it's a defunct AFAIK
                redo: function () {
