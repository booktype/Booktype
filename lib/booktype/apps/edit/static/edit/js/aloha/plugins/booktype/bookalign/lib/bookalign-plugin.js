		...
		toggleAlign: function ( domObj, property ) {
			...
			jQuery.each(rangeTree,function(cnt,el){
				if(el.type!=="none"){
					...
					console.log(jQuery(el.domobj));
					var selected_element = el.domobj;
					// I guess we are in table
					if(selected_element.nodeName!=='P'){
						var newRange = new GENTICS.Utils.RangeObject();
						newRange.startContainer = newRange.endContainer = selected_element;
						newRange.startOffset = 0;
						newRange.endOffset = selected_element.length;
						console.log(newRange);
						GENTICS.Utils.Dom.addMarkup(newRange,jQuery("<p></p>"));
						// and moving is done before the line is closed so no paragraph marks - let's add them 
						that.toggleAlign(domObj, property);
					}
					var currentAlignment = jQuery(selected_element).css( property );
					if ( currentAlignment === newAlignment ) {
						jQuery(selected_element).css( property, '' );
					} else {
						jQuery(selected_element).css( property, newAlignment );
					}
				}
			});
