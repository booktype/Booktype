...
define(
	...
	return Plugin.create('bookalign', {
		...
		toggleAlign: function () {
			...
			while(elements.indexOf(new_root.tagName)===-1){
			...
			if(new_root===root){
				...
				jQuery.each(rangeTree,function(cnt,el){
					if(el.type!=="none"){
						...
						if(selected_element.nodeType!==1){
							selected_element = root;
						}
					}
				});
			} else {
				selected_element = new_root;
			}
			var currentAlignment = jQuery(selected_element).css( 'text-align' );
			if ( currentAlignment === newAlignment ) {
				jQuery(selected_element).css( 'text-align', '' );
			} else {
				jQuery(selected_element).css( 'text-align', newAlignment );
			}
		}
