  define(['aloha', 'aloha/plugin', 'jquery',  'jquery19', 'ui/ui', 'aloha/ephemera', 'block/block', 'block/blockmanager', 'underscore', 'booktype', 'PubSub'], 
  		function(Aloha, Plugin, jQuery17,  jQuery, UI, Ephemera, block, BlockManager, _, booktype, PubSub) {
...
//                                $element.append(jQuery('<span class="heading-2">'+booktype._('title_author_content', "Author's name")+'</span>'));
         ...
                    BlockManager.registerBlockType('TitleBlock', TitleBlock);
                    ...
                    PubSub.sub('aloha.selection.context-change', function(m){
                      ...
                      if($block.length > 0) {
                        jQuery('DIV.contentHeader button.insertImage').prop('disabled', true);
                        jQuery('DIV.contentHeader button.insertLink').prop('disabled', true);
                        jQuery('DIV.contentHeader button.table-dropdown').prop('disabled', true);

                        jQuery('DIV.contentHeader button.alignLeft').prop('disabled', true);
                        jQuery('DIV.contentHeader button.alignRight').prop('disabled', true);
                        jQuery('DIV.contentHeader button.alignCenter').prop('disabled', true);
                        jQuery('DIV.contentHeader button.alignJustify').prop('disabled', true);

                        jQuery('DIV.contentHeader button.unorderedList').prop('disabled', true);
                        jQuery('DIV.contentHeader button.orderedList').prop('disabled', true);

                        jQuery('DIV.contentHeader .action.horizontalline').parent().addClass('disabled');
                        jQuery('DIV.contentHeader .action.pbreak').parent().addClass('disabled');

                        wasInHeading = true;
                      } else if(wasInHeading) {
                        jQuery('DIV.contentHeader button.insertImage').prop('disabled', false);
                        jQuery('DIV.contentHeader button.insertLink').prop('disabled', false);
                        jQuery('DIV.contentHeader button.table-dropdown').prop('disabled', false);

                        jQuery('DIV.contentHeader button.alignLeft').prop('disabled', false);
                        jQuery('DIV.contentHeader button.alignRight').prop('disabled', false);
                        jQuery('DIV.contentHeader button.alignCenter').prop('disabled', false);
                        jQuery('DIV.contentHeader button.alignJustify').prop('disabled', false);

                        jQuery('DIV.contentHeader button.unorderedList').prop('disabled', false);
                        jQuery('DIV.contentHeader button.orderedList').prop('disabled', false);

                        jQuery('DIV.contentHeader .action.horizontalline').parent().removeClass('disabled');
                        jQuery('DIV.contentHeader .action.pbreak').parent().removeClass('disabled');
