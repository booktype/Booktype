...
//                                $element.append(jQuery('<span class="heading-2">'+booktype._('title_author_content', "Author's name")+'</span>'));
                        ...
                        if(jQuery('.heading-date', $element).length == 0) {
                                ...
                                if(window.bodde.style.stylesData.article1.date)
                                  _s += '<h3 class="aloha-editable aloha-title-date">'+_date+'</h3>';

                                _s += '<h1 class="aloha-editable">'+_title+'</h1>';

                                if(window.bodde.style.stylesData.article1.author)
                                  _s += '<h2 class="aloha-editable">'+_author+'</h2>';

        		    				      	$element.append('<div style="">'+_s+'</div>');
        		    				      	break;
        				      		case 'style4':
                                var _s = '';

                                if(window.bodde.style.stylesData.article2.desc)
                                  _s += '<h3 class="aloha-editable aloha-title-description">'+_description+'</h3>';

                                if(window.bodde.style.stylesData.article2.author)
                                  _s += '<h2 class="aloha-editable">'+_author+'</h2>';

        		    				      	$element.append('<div style=""><h1 class="aloha-editable">'+_title+'</h1>'+_s+'</div>');
        		    				      	break;
        				      		case 'style6':
        		    				      	$element.append('<div style=""><h1 class="aloha-editable">'+_title+'</h1></div>');				      		
          				      		        break;
                      ...
                      update: function($element, postProcessFn) {
                      	...
                      	switch(style) {
                      		case 'style3':
		                      	  $container.empty();
                              console.log(window.bodde.style.stylesData);
                              if(window.bodde.style.stylesData.article1.date) {
        ...
        var _initHeadings = function(editable, unmodify) {
              ...
              var $span = jQuery('<div class="aloha-book-title"></div>');
       ...
                  BlockManager.registerBlockType('TitleBlock', TitleBlock);
                  ...
                  // User has changed style in the Style tab.
                    ...
                    // and setup new style attribute. This will make Title block refresh itself
         	        	jQuery.each(jQuery('div.aloha-book-title'), function(i, v) {
         	        		var b = BlockManager.getBlock(jQuery(v));

         	        		if(!_.isUndefined(b)) {
	         	        		b.attr('style', sid);
                        b.attr('rnd', Math.floor((Math.random()*10000)+1));                        
                      }
