			 ...
			 var TitleBlock = block.AbstractBlock.extend({
				      ...
				      init: function($element, postProcessFn) {			 	
                        ...
                        if(jQuery19('h1', $element).length == 0) {
                                $element.append(jQuery19('<h1>'+booktype._('title_heading_content', 'Heading content')+'</h1>'));
                        }

                        if(jQuery19('h2', $element).length == 0)
                                $element.append(jQuery19('<h2>'+booktype._('title_sub_heading_content', 'Sub heading content')+'</h2>'));

                        if(jQuery19('h3', $element).length == 0)
                                $element.append(jQuery19('<h3>'+booktype._('title_author_content', "Author's name")+'</h3>'));
         ...
                    BlockManager.registerBlockType('TitleBlock', TitleBlock);
					...
					Aloha.bind('aloha-editable-created', function($event, editable) {
		  				...
		  				if($h1.length > 0) {
		  					...
		  					if($parent.prop('tagName').toLowerCase() == 'span' && $parent.hasClass('bod-heading')) {
		  						...
		  						$h1.eq(0).parent().alohaBlock({'aloha-block-type': 'TitleBlock'});
		  					}
                  ...
                  jQuery(obj).find('.aloha-book-title').each(function() {
        		    ...
        		    var _get = function(tagName) {
 	        		...
 	        		var $newNode = jQuery('<span class="bod-heading"><h1>'+_get('h1')+'</h1><h2>'+_get('h2')+'</h2><h3>'+_get('h3')+'</h3></span>');
