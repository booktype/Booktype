define(
['aloha', 'aloha/plugin', 'jquery', 'booktype', 'PubSub'],
function (Aloha, plugin, jquery, booktype, PubSub) {
    'use strict';
        ...
        Aloha.bind('aloha-editable-created', function ($event, $editable) {
          if ($editable.obj && $editable.obj.attr('id') === 'contenteditor') {
            ...
            jquery('.contentHeader .btn-toolbar .fontsize-dropdown a').on('click', function ($event) {
              ...
              // publish message notification that change-fontsize element was clicked
              PubSub.pub('toolbar.action.triggered', {'event': $event});
            });

            jquery('button.btn.btn-default.btn-sm.fontSize').on('click', function ($event) {
