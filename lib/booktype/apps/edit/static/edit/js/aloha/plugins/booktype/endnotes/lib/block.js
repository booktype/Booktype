  ...
  function (Aloha, Plugin, jQuery, jQuery19, UI, Ephemera, block) {
    ...
    var EndnoteBlock = block.AbstractBlock.extend({
      ...
      recalculate: function () {
        ...
        var $endnotes = obj.find('sup[data-id]');
        ...
        $endnotesContainer.find('div[data-id]').each(function (idx, el) {
          ...
          if (!obj.find('sup[data-id="' + $el.attr('data-id') + '"]').length) {
            ...
            $orphansContainer.find('span:eq(' + ($orphansCount) + ')').parent().find('a').on('click', function (e) {
              $this._trashOrphan(e);
              return false;
            });
        ...
        $endnotes.each(function (idx, el) {
          ...
          if ($row.length > 0) {
            $row.find('span:first').html((idx + 1) + '.');
            return true;
          } else if (idx === 0) {
            $endnotesContainer.prepend($this._getRowHtml($el.attr('data-id'), idx + 1, ''));
          } else {
            $endnotesContainer.find('span:eq(' + (idx - 1) + ')').parent().after(
              $this._getRowHtml($el.attr('data-id'), idx + 1, '')
            );
          }
          $endnotesContainer.find('span:eq(' + (idx) + ')').parent().find('a').on('click', function (e) {
            $this._trash(e);
            return false;
          });
        });
      ...
      _trash: function (e) {
        ...
        if ($confirm === true) {
          var targetSup = obj.find('sup[data-id="' + e_id + '"]');
      ...
      _getRowHtml: function (dataID, index, endnoteText) {
        if (window.booktype.bookDir === 'rtl') {
          return '<div class="row elem" data-id="' + dataID + '">' +
                 '<div class="command col-md-1">' +
                 '<a href="#" class="pull-right"><i class="icon-trash"></i></a>' +
                 '</div>' +
                 '<div valign="top" class="endnoteText aloha-editable col-md-10">' + endnoteText + '</div>' +
                 '<span style="margin-right: 10px;" valign="top" class="pull-right">' + index + '.</span>' +
                 '</div>';
        }
        return '<div class="row elem" data-id="' + dataID + '">' +
               '<span style="margin-left: 10px;" valign="top" class="pull-left">' + index + '.</span>' +
               '<div valign="top" class="endnoteText aloha-editable col-md-10">' + endnoteText + '</div>' +
               '<div class="command col-md-1">' +
               '<a href="#" class="pull-right"><i class="icon-trash"></i></a>' +
               '</div>' +
