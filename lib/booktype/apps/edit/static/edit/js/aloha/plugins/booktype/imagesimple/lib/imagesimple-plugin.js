  		...
  		function(Aloha, Plugin, jQuery,  jQuery19, UI, Console, Ephemera, block, BlockManager, booktype, _) {
		    ...
		    var ImageBlock = block.AbstractBlock.extend({
			      ...
			      init: function($element, postProcessFn) {	
			      		...
			      		var $n = jQuery('<div class="caption aloha-editable">'+$desc.html()+'</div>');
         	...
         	// Show images in the Image Manager Dialog
  			...
  			var _initImages = function(editable) {
               	 jQuery('img', editable.obj).each(function() {
               	 	...
               	 	var $g = $img.closest('div.group');

               	 	if($g.length > 0) {
	               	 	var $p = $g.find('p.caption');
  			...
  			return Plugin.create('imagesimple', {
  				...
  				makeClean: function(obj) {
  					jQuery(obj).find('DIV.group').each(function() {
  						var $div = jQuery(this);
  						var $d = jQuery('<div class="group"></div>');

  						$d.append($div.find('img'));

  						var $desc = $div.find('div');
  						if($desc.length > 0) {
  							var $d0 = jQuery('<p class="caption">'+$desc.html()+'</p>');
  							$d.append($d0);
  						}
  						$div.replaceWith($d);
  					});
