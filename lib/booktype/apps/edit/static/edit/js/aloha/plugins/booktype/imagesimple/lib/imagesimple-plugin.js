    ...
    return Plugin.create('imagesimple', {
      makeClean: function (obj) {
        ...
        jQuery(obj).find('DIV.group_img').each(function () {
          var $group_img = jQuery(this);

          // create new group_img
          var $new_group_img = jQuery('<div class="group_img"></div>');
          $new_group_img.css('text-align', $group_img.css('text-align'));
          $new_group_img.append($group_img.find('div.image'));

          // find caption if it exists
          var $div_caption_small = $group_img.find('div.caption_small');

          if ($div_caption_small.length > 0) {
            var $new_caption_small = jQuery('<div class="caption_small">' + $div_caption_small.html() + '</div>');
            $new_group_img.append($new_caption_small);
          }

          // swap new group_img with new one
          $group_img.replaceWith($new_group_img);
      ...
      init: function () {
        ...
        jQuery19('.insert-image', $manager).on('click', function ($event) {
          ...
          var $group_img = jQuery('<div class="group_img"></div>');
          var $div_image = jQuery('<div class="image"></div>');

          $div_image.append($image);
          $group_img.append($div_image);

          if (description !== '') {
            $group_img.append(jQuery('<div class="caption_small">' + description + '</div>'));
          }
          ...
          GENTICS.Utils.Dom.insertIntoDOM($group_img, range, $editable.obj);
