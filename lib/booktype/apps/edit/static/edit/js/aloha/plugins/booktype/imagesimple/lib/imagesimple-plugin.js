define(['aloha', 'aloha/plugin', 'jquery',  'jquery19', 'ui/ui', 'aloha/console', 'aloha/ephemera', 'block/block', 'block/blockmanager', 'booktype', 'underscore', 'image/image-plugin'],
  function (Aloha, Plugin, jQuery,  jQuery19, UI, Console, Ephemera, block, BlockManager, booktype, _, image) {
    var editable;
    var range;
    var fileUpload;
    ...
    var getAttachment = function (mediaID) {
      return _.find(attachments, function (a) { return a.id === _.toNumber(mediaID); });
    };
...
//          }
   ...
        BlockManager.registerBlockType('ImageBlock', ImageBlock);
        ...
        jQuery19('.insert-image', $manager).on('click', function () {
          ...
          var $a = jQuery('<img alt="" src=""/>').prop('src', 'static/' + selectedImage.name);
          $a.prop('alt', title);
