...
(function(win, jquery, _) {
  ...
  win.booktype.editor.media = function() {
    ...
    var MediaView = Backbone.View.extend({
         ...
         render: function() {
            ...
            var rows = this.model.map(function(item) {
                         return $this.templRow({'name': item.get('name'),
                                               'id': item.get('id'), 
    ...
    var loadAttachments = function() {
      ...
      win.booktype.sendToCurrentBook({"command": "attachments_list"}, 
            function(data) {
                ...
                mediaItems.reset(_.map(data.attachments, 
                     function(itm) {
                        return new MediaItem(itm);   
                     })
                );
    ...
    var _init = function() {
        jquery("#button-media").on('change', function(e) { Backbone.history.navigate('media', true); });
