...
(function (win, jquery, _) {
  ...
  win.booktype.editor = (function () {
      ...
      win.booktype.sendToCurrentBook({
        ...
        function (dta) {
          ...
          activePanel.hide(function () {
            ...
            //jquery('#content').html(dta.content);
    ...
    var _initEditor = function (settings) {
      ...
      // #bobbob
      // initialize bobbob BooktypeApi bridge
      // we initialise it once and change only documentID when chapter is opened
      win.BooktypeApiInstance = new win.BooktypeApi(
        {
          documentID: null,
          direction: window.booktype.bookDir,
          language: window.booktype.bookLang,
          autosave: window.booktype.editor.autosave,
          bookID: window.booktype.currentBookID,
          booktypeURL: window.booktype.baseURL,
          otEnabled: true,
          otServerWsUrl: 'ws://' + window.booktype.baseURL.split('://')[1].split(':')[0] + ':8765'
        }
      );
      // create EditorController instance
      win.EditorControllerInstance = new win.EditorController(
        win.BooktypeApiInstance,
