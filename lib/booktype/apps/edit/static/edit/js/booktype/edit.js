...
(function (win, jquery, _) {
  ...
  win.booktype.editor.edit = (function () {
    ...
    var _show = function () {
      jquery('body').addClass('bobbob-active');
      ...
      // t2 = win.booktype.ui.getTemplate('templateAlohaContent');
      ...
      // #bobbob
      // create BobbobEditor component
      if (
        win.EditorControllerInstance.getDocumentID() &&
        win.EditorControllerInstance.getDocumentID() !== chapterID
      ) {
        win.EditorControllerInstance.destroyEditor();
      }

      setTimeout(function () {
        win.EditorControllerInstance.initEditor(chapterID)
      }, 0);

    };

    var _hide = function (callback) {
      jquery('body').removeClass('bobbob-active');
