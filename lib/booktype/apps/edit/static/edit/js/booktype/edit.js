  ...
  win.booktype.editor.edit = (function () {
    ...
    var _checkIfModified = function (callback, errback) {
      ...
      if (!_.isUndefined($editor) && $editor && $editor.isModified()) {
        ...
        if (result) {
          win.booktype.editor.edit.saveContent(function () { callback(); });
          return true;
        }
      }
    ...
    var _show = function () {
      ...
      $("#saveContentWithCommentModal button").click(function(){
        if (this.dataset.action === "save"){
          var comment = $("#saveContentWithCommentModal textarea[name=comment]").val();
          win.booktype.editor.edit.saveContent({comment: comment}); 
        } 
        $("#saveContentWithCommentModal textarea[name=comment]").val('');
        $("#saveContentWithCommentModal").modal("hide");
      });
    ...
    var _hide = function (callback) {
      _checkIfModified(function () {
        ...
        function () {
          setTimeout(function () { jquery('.btn-toolbar label').removeClass('active'); }, 0);
        }
      );
    };
