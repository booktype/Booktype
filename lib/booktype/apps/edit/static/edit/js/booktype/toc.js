  ...
  win.booktype.editor.toc = (function () {
    ...
    _.extend(TOC.prototype, Backbone.Events, {
      ...
      loadFromArray: function (arr) {
        ...
        console.log('LOAD FROM ARRAY', arr);
    ...
    var CREATE_CHAPTER = function (item) {
      ...
      jquery.each(win.booktype.editor.data.statuses, function (i, v) {
        ...
        if(parseInt(item.get('status'), 10) === parseInt(v[0], 10)){
          $l.push(jquery("<li class='selected'/>").wrapInner($a));
        }else{
          $l.push(jquery("<li/>").wrapInner($a));
        }
      });

      jquery(".btn-toolbar .btn-group:first-child ul.status-options", templateChapter).append($l);
    ...
    var TOCWidget = function () {
      ...
      this.refreshItem = function (item) {
        ...
        // lock/unlock
        if (item.isLocked()) {
          jquery(".unlocked", $li).addClass("hide");
          jquery(".locked", $li).removeClass("hide");
        } else {
          jquery(".unlocked", $li).removeClass("hide");
          jquery(".locked", $li).addClass("hide");
        }
      };
