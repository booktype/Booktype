...
(function (win, jquery, _) {
  ...
  win.booktype.editor.toc = (function () {
    ...
    var reoderTOC = function (newOrder) {
      ...
      _.each(newOrder, function (itemID) {
        var c = win.booktype.editor.data.chapters.getChapterWithID(itemID);
    ...
    _.extend(TOC.prototype, Backbone.Events, {
      ...
      getChapterIndexWithID: function (cid) {
        for (var i = 0; i < this.chapters.length; i++) {
          if (this.chapters[i].get("chapterID") === cid) {
            return i;
          }
        }
      ...
      getChapterWithID: function (cid) {
        if (String(cid).charAt(0) === "s") {
          return _.find(this.chapters, function (elem) { return elem.get("chapterID") === cid; });
        } else {
          return _.find(this.chapters, function (elem) { return elem.get("chapterID") === parseInt(cid); });
        }
      },
    ...
    var CREATE_SECTION = function (item) {
      ...
      sec.attr("data-id", item.get("chapterID"));
      sec.attr("id", "list_" + item.get("chapterID"));
    ...
    var CREATE_CHAPTER = function (item) {
      ...
      templateChapter.attr("data-id", item.get("chapterID"));
      templateChapter.attr("id", "list_" + item.get("chapterID"));
      ...
      jquery("A.option-rename", templateChapter).click(function () {
        jquery("#renameChapter INPUT[name=chapter_id]").val(item.get("chapterID"));
    ...
    var TOCWidget = function () {
      ...
      this.removeItem = function (item) {
        var $li = jquery("LI[data-id=" + item.get("chapterID") + "]");
    ...
    var _doDeleteSection = function (sectionID) {
      var $d = jquery.Deferred();
    ...
    var _doRenameChapter = function (chapterID, name) {
      var $d = jquery.Deferred();
      ...
      win.booktype.sendToCurrentBook({
        ...
        function (data) {
          var c = win.booktype.editor.getChapterWithID(chapterID);
      ...
      if (win.booktype.editor.isEnabledTab("toc", "hold")) {
        ...
        t2.onActivate = function () {
          ...
          _.each(win.booktype.editor.data.holdChapters.chapters, function (v) {
            var $d = jquery("<li><div>" + v.get("title") + "</div></li>");
    ...
    var _init = function () {
      jquery("#button-toc").on("change", function (e) { Backbone.history.navigate("toc", true); });
      ...
      jquery("#removeChapter .operation-remove").on("click", function () {
        if (jquery("#removeChapter INPUT[name=understand]:checked").val() === "on") {
          var chapterID = jquery("#removeChapter INPUT[name=chapter_id]").attr("value");
      ...
      jquery("#removeSection .operation-remove").on("click", function () {
        if (jquery("#removeSection INPUT[name=understand]:checked").val() === "on") {
          ...
          var _do = win.booktype.editor.toc.doDeleteSection(sectionID);
      ...
      jquery("#renameChapter BUTTON.operation-rename").on("click", function () {
        ...
        var chapterID = jquery("#renameChapter INPUT[name=chapter_id]").attr("value");

        if (jquery.trim(name) === "") { return; }

        var _do = win.booktype.editor.toc.doRenameChapter(chapterID, name);
