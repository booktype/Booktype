...
(function (win, jquery, _) {
  ...
  win.booktype.editor.toc = (function () {
    ...
    var CREATE_CHAPTER = function (item) {
      ...
      if(item.get("lock") == 1 || item.get("lock") == 2){
        hideChapter(templateChapter);
      }
      ...
      // lock
      jquery("A.option-lock-by-perms", templateChapter).on("click", function () {
        win.booktype.ui.notify("Sending data");
        item.set('lock', 2);
        win.booktype.sendToCurrentBook({
      ...
      // unlock
      jquery("A.option-unlock", templateChapter).on("click", function () {
        win.booktype.ui.notify("Sending data");
        item.set('lock', 0);
        win.booktype.sendToCurrentBook({
    ...
    var hideChapter = function(template){
      jquery('div:first a:first i', template).attr("class", "toc-icon-chapter-locked");
      jquery('div.btn-toolbar .btn-group', template).hide();
      jquery('div.btn-toolbar .locked', template).show();

      var $lock_html = '<div class="btn-group"><span class="label">Locked</span></div>';
      jquery('div.btn-toolbar', template).append($lock_html);
    };

    var showLockedChapter = function(template){
      jquery('div:first a:first i', template).attr("class", "toc-icon-chapter");
      jquery('div.btn-toolbar .btn-group', template).show();
      jquery('div.btn-toolbar .locked', template).hide();
      jquery('div.btn-toolbar .btn-group:last', template).remove();
    };
      ...
      if (win.booktype.editor.isEnabledTab("toc", "hold")) {
        ...
        t2.onActivate = function () {
          var tb = jquery("section[source_id=hold-tab]"),
            holdBtnGroup = win.booktype.ui.getTemplate("templateHoldButtonGroup");
          ...
          _.each(win.booktype.editor.data.holdChapters.chapters, function (v) {
            ...
            $d.hover(function () {
              ...
              //console.log(v.get('lock'));

              if(v.get('lock') == 1 || v.get('lock') == 2){
                jquery('div.btn-group:last button.unlocked', holdBtnGroup).hide();
                jquery('div.btn-group:last span.locked', holdBtnGroup).show();
              } else {
                jquery('div.btn-group:last button.unlocked', holdBtnGroup).show();
                jquery('div.btn-group:last span.locked', holdBtnGroup).hide();
              }
      ...
      win.booktype.subscribeToChannel("/booktype/book/" + win.booktype.currentBookID + "/" + win.booktype.currentVersion + "/",
        function (message) {
          ...
          var funcs = {
            ...
            "chapter_lock": function () {
              var item = win.booktype.editor.data.chapters.getItemWithChapterID(message.chapterID);
              var chapter = jquery('div.templateTOCContent div.toc li[data-chapter-id="' +  item.get("chapterID") + '"]')
