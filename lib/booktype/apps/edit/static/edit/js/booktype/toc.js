  ...
  win.booktype.editor.toc = (function () {
    ...
    var CREATE_CHAPTER = function (item) {
      ...
      if(item.get("lock") == 1 || item.get("lock") == 2){
        jquery('div:first a:first i', templateChapter).attr("class", "toc-icon-chapter-locked");

        // hide edit bar for unlocked chapters and show for locked
        jquery('div.btn-toolbar .btn-group', templateChapter).hide();
        jquery('div.btn-toolbar .btn-group', templateChapter).slice(-1).show();

        // todo remove hardcoded html
        var $lock_html = '<div class="btn-group"><span class="label">Locked</span></div>'
        jquery('div.btn-toolbar', templateChapter).append($lock_html);
      }
      ...
      jquery("A.option-lock-by-perms", templateChapter).on("click", function () {
        ...
        win.booktype.sendToCurrentBook({
          ...
          function () {
            ...
            //// Trigger event
            //jquery(document).trigger("booktype-document-hold", [item]); // todo
          }
        );
