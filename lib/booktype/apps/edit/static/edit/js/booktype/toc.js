...
(function (win, jquery, _) {
  ...
  win.booktype.editor.toc = (function () {
    ...
    var CREATE_CHAPTER = function (item) {
      ...
      var renderedChapter = _.template(jquery('#templateTOCChapter').html())({
      ...
      // edit lock
      if (item.isUnderEdit()) {
        if (!jquery(".edit-by", templateChapter).length) {
          jquery(".btn-group", templateChapter).hide();
          jquery("a.operation-edit", templateChapter).css("pointer-events", "none");

          var icon = jquery("<span>").attr({class: "icon-pencil"});
          var label = jquery("<span>").attr({
            class: "label label-primary pull-left",
            style: "margin-right: 5px;"
          }).html(icon);
          label.append("&nbsp;" + item.get("editBy"));

          // create username profile link
          var profileLink = jquery("<a>").attr({
            href: "/accounts/" + item.get("editBy") + "/",
            class: "edit-by",
            target: "_blank"
          }).html(label);
          jquery(".btn-toolbar .toc-chapter-short-labels", templateChapter).prepend(profileLink);
          ...
          var detailProfileLink = jquery("<a>").attr({
            ...
            class: "edit-by",
            target: "_blank"
          }).html(icon.clone()).append("&nbsp;" + item.get("editBy"));
          // TODO remove this hardcoded div and hardcoded custom labels
          jquery(".toc-chapter-detail", templateChapter).prepend(jquery("<div class=\"edit-by\"></div>").html(detailProfileLink));
        }
      }
    ...
    var TOCWidget = function () {
      ...
      this.refreshItem = function (item) {
        ...
        // lock/unlock
        if (item.isLocked()) {
          jquery(".display-unlocked", $li).addClass("hide");
          jquery(".display-locked", $li).removeClass("hide");
        } else {
          jquery(".display-unlocked", $li).removeClass("hide");
          jquery(".display-locked", $li).addClass("hide");
        }

        // edit lock
        if (item.isUnderEdit()) {
          // If chapter hasn't allready editby information
          if (!jquery(".edit-by", $li).length) {
            jquery(".btn-group", $li).hide();
            jquery("a.operation-edit", $li).css("pointer-events", "none");
            ...
            jquery(".btn-toolbar .toc-chapter-short-labels", $li).prepend(profileLink);
            ...
            // TODO remove this hardcoded div and hardcoded custom labels
            jquery(".toc-chapter-detail", $li).prepend(jquery("<div class='edit-by'></div>").html(detailProfileLink));
          }
        } else {
          jquery(".btn-group", $li).show();
          jquery("a.operation-edit", $li).css("pointer-events", "");
          jquery(".edit-by", $li).remove();
        }
    ...
    var _toc = function () {
      ...
      jquery("#detail-switch").click(function () {
        ...
        jquery(".sortable li .btn-toolbar .toc-chapter-short-labels").toggleClass("hide");
      });
        ...
        function (message) {
          ...
          var funcs = {
            ...
            "chapter_state": function () {
              ...
              item.set('state', message.state);
              item.set('editBy', message.username || null);

              if (win.booktype.editor.getActivePanel().name !== "toc") { return; }

              tocWidget.refreshItem(item);
            },
            ...
            "chapter_create": function () {
              var c = new Chapter({
                ...
                "parentID": message.chapter[6],
                "tocID": message.chapter[7]
              });
