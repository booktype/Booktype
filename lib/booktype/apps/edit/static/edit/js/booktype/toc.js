  ...
  win.booktype.editor.toc = (function () {
    ...
    var _show = function () {
      ...
      jquery(document).on("hidden.bs.modal", "#section-settings", function () {
        ...
        $dialog.find("form input[name^='hide_in_outputs']").attr("disabled", true);
      });

      // on section settings modal shown
      jquery(document).on("shown.bs.modal", "#section-settings", function () {
        ...
        win.booktype.sendToCurrentBook({
          ...
          function (data) {
            if (data.result) {
              ...
              _.each(settings["hide_in_outputs"], function (value, key) {
                $dialog.find("input#id_" + key).prop("checked", value);
              });

              $dialog.find("form input[name^='hide_in_outputs']").attr("disabled", false);
            }
          }
        );
      });
