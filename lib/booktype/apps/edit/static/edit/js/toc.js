...
(function(win, jquery) {
  ...
  win.booktype.editor.toc = function() {
      ...
      var getStatusName = function(statusID) {
                             var $s = jquery.grep(win.booktype.editor.statuses(), function(v, i) { return v[0] == statusID;});
      ...
      var CREATE_CHAPTER = function(item) {
                             ...
                             // Insert status names
                             var $l = [];
                             jquery.each(win.booktype.editor.statuses(), function(i, v) {
                             ...
                             jquery('A.option-delete', templateChapter).click(function() {
                                 ...
                                 jquery('#removeChapter INPUT[name=chapter_id]').val(item.chapterIDb);
                                  /*
                                 var r = confirm('Really want to delete it?');

                                 if(r == true) {
                                    win.booktype.ui.notify('Deleting chapter.');
                                    ...
                                    win.booktype.sendToCurrentBook({"command": "chapter_delete", 
                                                                    ...
                                                                    function() { 
                                                                      ...
                                                                      jquery('OL.sortable LI[data-id='+item.chapterID+']').remove();
                                                                    });
                                 }
                                 */
                                 return false;
      ...
      var TOCWidget = function() {
                         this.draw = function() {
                            var lst = win.booktype.editor.chapters().chapters;
                            var s = [];
                            var s2 = [];
                            var inSection = false;
                            var sec = null;
        ...
        jquery('ol.sortable').nestedSortable({
            ...
            stop: function(event, ui) {             
                    ...
                    win.booktype.sendToCurrentBook({"command": "chapters_changed", 
                                                   ...
                                                   function() {
    //                                                 $.booki.ui.notify();  toc.refreshLocks(); holdChapters.refreshLocks();
                                                   });
                    return true;
            }
        });
        ...
        var _show = function() {
          win.booktype.ui.notify('Ucitavam');
          //jquery( ".ui-draggable" ).draggable({ containment: 'window', cursor: "crosshair", scroll: false, scope: 'media'});
          ...
          jquery('DIV.right-tablist UL.navigation-tabs').append('<li><a href="#" id="hold-tab"><i class="big-icon-hold"></i></a></li>');

          win.booktype.editor.rightPanel('.right-tablist li #hold-tab', function() {
        ...
        var _hide = function() {
          ...
          jquery('DIV.right-tablist UL.navigation-tabs #hold-tab').remove();
        ...
        var _init = function() {
            ...
            jquery("#removeChapter .btn-primary").on('click', function() {
              console.log(jquery("#removeChapter INPUT[name=chapter_id]"));
              console.log(jquery("#removeChapter INPUT[name=chapter_id]").attr("value"));
              console.log('BRISE SE SRCE MOJE');
              jquery("#removeChapter").modal('hide');
            })
              ...
              // CHECK IF WE ARE IN THIS PANEL, ONLY THEN DO TRANSFORMATIONS
                 var funcs = {
                     "change_status": function() {
                        var chap = win.booktype.editor.chapters().getChapterWithID(message.chapterID);
