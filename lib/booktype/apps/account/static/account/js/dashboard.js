...
(function (win, $) {
  ...
  win.booktype.dashboard = {
    ...
    init: function () {
      ...
      var allowedTypes = ['application/epub+zip']; // just epub for now
      ...
      $(document).on('drop', '#drag-area', function (e) {
        ...
        $('#book-file')[0].files = files;
      });

      $(document).on('change', '#book-file', function (e) {
        if (this.files.length > 0) {
          $('.book_filename b').html(this.files[0].name);
      ...
      $(document).on('send_book', '#book-file', function (e) {
        // clear all alerts
