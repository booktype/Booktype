...
jquery('group').ready(function() {
    ...
    jquery("button.join-group-button").click(function() {
        var groupName = jquery(this).attr('group-name');
        win.booktype.initCSRF();

        jquery.post("/newaccounts/group/"+groupName+"/", {
            task: "join-group"
        }).done(function(data){
            window.location = "/newaccounts/group/"+groupName+"/";
            });
        });
