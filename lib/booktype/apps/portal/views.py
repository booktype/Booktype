...
class FrontPageView(views.SecurityMixin, PageView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_list'] = User.objects.exclude(is_active=False).order_by('-date_joined')[:2]
...
class PeoplePageView(views.SecurityMixin, PageView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['all_people'] = User.objects.exclude(is_active=False).extra(select={'lower_username': 'lower(username)'}).order_by('lower_username')
        ...
        context['new_people'] = User.objects.exclude(is_active=False).order_by('-date_joined')[:4]
